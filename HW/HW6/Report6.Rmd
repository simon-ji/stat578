...
densplot(coef_sample_1[, paste("prob[",Dosunmu_index,"]",sep="")], main = "Density of Probability for Ayo Dosunmu")
```
...
Give the bayes factor is between 20 to 150, we can say that the data has **Strong** evidence that $\beta_F$ > $\beta_G$.
...
```
```{r}
coef_sample_2 <- coda.samples(model_2, c("beta_pos","beta_ht","prob","FGM_rep", "sigma_epsilon"),
                              n.iter = 50000)
effectiveSize(coef_sample_2[,c("beta_pos[1]", "beta_pos[2]", "beta_pos[3]", "beta_ht", "sigma_epsilon")])
```
#### (ii)
```{r}
...
mean(Tchirep >= Tchi)
```
The posterior predictive p-value is extremely small. Thus this could could have overdispersion problem.

### (e)
#### (i)
```{r}
p_sample <- matrix(0, nrow = nrow(BLK_rep), ncol = nrow(perf_data))
for(s in 1:nrow(BLK_rep)){
...
Adonis played in center position and was 84 height. He played 225 minutes but was blocked only 1 shot. Similar to him, Samba also played in center position and was 84 height. For 86 minutes he played, 10 shots were blocked. Thus Adonis real performance is exceptional compared to our model. Thus the p-value is very high.
